...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fireNotification = useCallback((message: Message) => {
    ...
    const senderName =
      message.sender?.name || message.sender?.phone || "Alguien";
    ...
    const notifOptions: NotificationOptions & { renotify?: boolean } = {
      ...
      silent: (() => {
        try {
          return !JSON.parse(
            localStorage.getItem("nexttalk_setting_notif_msgTone") ?? "true"
          );
        } catch {
          return false;
        }
  ...
  useEffect(() => {
    ...
    socket.on("new_message", (message: Message) => {
      ...
      const notifEnabled = (() => {
        try {
          return JSON.parse(
            localStorage.getItem("nexttalk_setting_notif_messages") ?? "true"
          );
        } catch {
          return true;
        }
  ...
  useEffect(() => {
    if (socketRef.current?.connected && chats.length > 0) {
      chats.forEach((chat) => socketRef.current?.emit("join_chat", chat._id));
    }
  }, [chats]);
  ...
  const sendMessage = async (
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        // El remitente agrega el mensaje localmente de inmediato;
        // el socket "new_message" tiene deduplicación para evitar duplicado
        setMessages((prev) => ({
          ...prev,
